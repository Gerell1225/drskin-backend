...
function sanitizeAscii(s: string, max = 45) {
...
orders.post("/cancel-items", async (req, res) => {
  try {
    ...
    const resp: any = { cancelled_count: result?.cancelled_count ?? 0, refundable_mnt: result?.refundable_mnt ?? 0 };
    ...
    if (auto_refund) {
      ...
      if (!itemPayments || itemPayments.length === 0) {
        ...
        for (const p of itemPayments) {
          try {
            const data = await qpayRefund(p.external_ref as string, reason ?? "Item cancelled");
...
orders.post("/cancel-order", async (req, res) => {
  try {
    ...
    for (const c of charges ?? []) {
      try {
        const data = await qpayRefund(c.external_ref as string, reason ?? "Order cancelled");
