...
// helper
const sanitize = (s, max = 45) => s.replace(/[^A-Za-z0-9_]/g, "").slice(0, max);
...
qpay.post("/invoice", async (req, res) => {
    try {
        ...
        if (order.amount_due_mnt <= 0)
            return res.status(400).json({ error: "nothing_to_charge" });
...
qpay.post("/invoice-item", async (req, res) => {
    try {
        ...
        if (item.status !== "active")
            return res.status(400).json({ error: "item_not_active" });
        const sender_invoice_no = sanitize(item_id);
        const branchCode = `BR-${item.booking_orders.branch_id.slice(0, 8)}`;
...
qpay.post("/callback", async (req, res) => {
    try {
        ...
        if (!payment_id)
            return res.status(400).json({ ok: false, error: "payment_id missing" });
        ...
        if (isPaid && order_id) {
            ...
            const { data: paid } = await supabaseAdmin
                ...
                .eq("kind", "charge")
                .eq("status", "paid");
            const totalPaid = (paid ?? []).reduce((s, r) => s + (r.amount_mnt ?? 0), 0);
